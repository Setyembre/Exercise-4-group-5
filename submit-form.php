...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($count > 0) {
        ...
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO users (lastname, firstname, middlename, email, contact_number, age, address, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die("Prepare failed: " . $conn->error);
        }
        $stmt->bind_param("ssssisis", $lastname, $firstname, $middlename, $email, $contact_number, $age, $address, $gender);
